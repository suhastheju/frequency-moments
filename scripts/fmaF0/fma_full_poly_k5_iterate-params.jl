...
function polyhash2(string::AbstractString, seed::Int, bits::Int)
    maxval = 2^bits
    hash = 0
    for char in collect(string)
        hash += mod( Int(char)^seed + 42, maxval)
    end
    ...
    open(file) do filehandle
        for line in eachline(filehandle)
            for k in 1:length(seeds)
                tail_length = trailing_zeros(polyhash2(line, seeds[k], 30))
...
@showprogress for num_hashes in logspace(6, 1, 6)
...
file_name = string("results/", "results_fma_poly_k5_iterate-params.csv")
